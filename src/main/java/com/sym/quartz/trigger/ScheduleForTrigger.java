 ...
 */
public class ScheduleForTrigger {
    public static void main(String[] args) throws SchedulerException {
        Date now = new Date();


        // 待执行的两个Job实例
        JobDetail jobDetailOne = JobBuilder.newJob(JobForTrigger.class).withIdentity("疾风剑豪", "刺客").withDescription("面对疾风吧").build();
        JobDetail jobDetailTwo = JobBuilder.newJob(JobForTrigger.class).withIdentity("灵魂烈焰", "法师").withDescription("面对疾风吧").build();
